...
- **Web** — Primary. Static documentation and marketing site (Astro, Tailwind CSS v4).
...
Developer tools earn trust through clarity, not decoration. The target audience — small dev teams using AI coding tools — values scannability, readability, and speed. A minimal aesthetic with a single vibrant accent creates visual interest without visual noise. Dark-first design matches the environment developers already work in (editors, terminals).
...
- Use CSS custom properties for all theme tokens, toggled via a `data-theme="dark"` / `data-theme="light"` attribute on `<html>`.
- Default to dark mode. Respect `prefers-color-scheme` media query for initial load.
...
- **Border radius:** 8px
- **Font:** Body size (1rem), weight 500
- **Hover:** `primary-400` (#A78BFA), slight lift (`translateY(-1px)`, shadow elevation 2)
- **Active:** `primary-600` (#7C3AED), no lift
...
- **Text:** `neutral-100` (dark) / `neutral-800` (light)
- **Hover:** `neutral-800` background (dark) / `neutral-100` background (light), border `neutral-600`
- **Active:** `neutral-700` background
- **Disabled:** 40% opacity

#### Tertiary / Ghost Button
- **Background:** transparent, no border
- **Text:** `primary-500`
- **Hover:** `primary-900` background
- **Active:** `primary-500` at 20% opacity background

#### Button Sizes

| Size | Padding | Font Size | Min Height |
|------|---------|-----------|------------|
| Small | 8px 12px | 0.875rem (14px) | 32px |
| Default | 12px 20px | 1rem (16px) | 40px |
| Large | 14px 28px | 1.125rem (18px) | 48px |

### Form Elements

#### Input Fields
- **Background:** `neutral-900` (dark) / `#FFFFFF` (light)
- **Border:** 1px solid `neutral-700` (dark) / `neutral-300` (light)
- **Border radius:** 8px
- **Padding:** `space-3` (12px) vertical, `space-4` (16px) horizontal
- **Text:** `neutral-100` (dark) / `neutral-900` (light)
- **Placeholder:** `neutral-500`
- **Focus:** Border changes to `primary-500`, 2px outline ring in `primary-500` at 25% opacity
- **Error:** Border changes to `error` (#F87171), helper text in `error`
- **Disabled:** 50% opacity, `neutral-800` background

#### Labels
- **Font:** Body Small (0.875rem), weight 500
- **Color:** `neutral-200` (dark) / `neutral-700` (light)
- **Spacing:** `space-2` (8px) below label, above input

#### Helper Text
- **Font:** Body Small (0.875rem), weight 400
- **Color:** `neutral-400` (default), `error` (validation error), `success` (valid)
- **Spacing:** `space-2` (8px) above, below input
...
- **Chevron icon:** `neutral-400`, 16px
- **Dropdown menu:** `neutral-900` background, `neutral-700` border, 8px radius, shadow elevation 3
...
- **Shadow:** None by default (dark mode relies on border contrast). Light mode uses elevation 1.
- **Hover (interactive cards):** Border shifts to `neutral-600` (dark) / `neutral-400` (light), subtle shadow elevation 2

### Navigation

#### Header / Navbar
- **Background:** `neutral-950` at 80% opacity with `backdrop-filter: blur(12px)`
- **Height:** 64px
- **Position:** Fixed top, full width
- **Border bottom:** 1px solid `neutral-800`
- **Logo/wordmark:** `neutral-100`, left-aligned
- **Nav links:** `neutral-300`, weight 400. Hover: `neutral-100`. Active: `primary-400`, weight 500
- **Mobile:** Hamburger menu icon at 24px. Fullscreen overlay menu on `neutral-950` background.

#### Active State
- Text color shifts to `primary-400`
- Optional: 2px bottom border in `primary-500` for tab-style navigation
...
- **Recommended library:** [Lucide Icons](https://lucide.dev/) — MIT-licensed, consistent stroke-based set, excellent coverage for developer tool UIs
