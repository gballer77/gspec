...
Categories: stack, architecture, style, practice, feature (checkbox false positive/negative), plan (task vs code), orphan capability, and profile (treat conservatively). For orphan capabilities, build a capability map (route + handler + UI + test clusters a *user* would call a feature) and filter out internal plumbing, utilities, and dev tooling.
