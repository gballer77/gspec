...
## Return contract
Return **only** the fenced ```json build-plan block defined by the `gspec-orchestrator` skill — ordered `waves`, each a list of `{ "label", "instruction" }` scopes, where the `instruction` is the self-contained brief handed verbatim to one implementer (naming the feature/plan files and task IDs in scope). No prose before or after the block. If there is no unchecked work in scope, return `{ "waves": [] }`.
