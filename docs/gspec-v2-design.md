...
Tools: `Read, Grep, Glob` **only** — no Write/Edit (they judge, they don't fix). Model: strong.
...
intake(interview)
  ...
  → implement     [ implementer — orchestrator picks scope (per feature/phase), fan-out independent scopes on [P] ]
