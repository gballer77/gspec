...
The library lives in the user's **home** `~/.gspec/` (distinct from a project's own `.gspec/` runtime folder). Only these four spec types are templated; `profile.md` and `architecture.md` are inherently project-specific and are never seeded from a template. A writer never discovers the library itself — it has no shell to expand `~` with — so whoever holds one resolves it and passes **absolute paths** into the writer's brief: the interactive command (which also offers the match and lets the user choose), or the build driver in an autonomous run (which lists the stage's candidates and leaves the choice to the writer). Either way the isolated writer adapts the template and records provenance. Validators are unchanged — templates seed authoring, not QA.
...
| `architecture-writer` | architect + conventions + agnosticism | brief + specs → `architecture.md` (+ `architecture/<name>.md` per module when the Modules table has >1 row) | architect, audit |
...
- **Resumable** via an on-disk run manifest — crash on phase 40 of 60, restart from 40. **Resume re-enforces, never skips, a failed gate:** a stage recorded `failed` (or crashed `running`) re-validates its on-disk deliverable FIRST on resume — honoring any hand-edit the user made to unblock — instead of skipping because the output file exists; skip-if-present is reserved for a genuinely pre-existing spec on a stage that never ran (`priorStatus === 'pending'`). Resume keys on the manifest's recorded status, not on output-file existence.
- **QA gates default on in the build too** (skippable via `--no-qa`). **Self-healing gate:** on a validator failure, the writer gets a revision attempt from the verdict — one by default, `--qa-retries <n>` sets the budget (0 = fail on first verdict; honored at resume time so a paused gate can retry with more attempts); still failing → pause/flag (never ships a bad spec), with the failing verdict printed in full and kept in `.gspec/build/last-failure.md` + the stage's `detail` in the manifest. **Severity-gated (reachable exit):** the driver grades the verdict (`gradeVerdict`) and enforces the gspec-qa contract that a spec FAILs only on a `blocker`/`major` finding — a `FAIL` whose findings are all `[minor]`/`[nit]` is passed with **advisory notes** (kept in `qa-failures.md`, tagged `ADVISORY`), so the loop has a state it can actually reach instead of chasing precision nits until the budget drains. Conservative: any blocker/major, or an untagged verdict, keeps the FAIL. **Durable QA log:** every failing verdict a run observes — not just the terminal one, but the intermediate ones a self-heal recovered from — is also appended in full to `.gspec/build/qa-failures.md` (via the driver's `recordFeedback` choke point plus the terminal fold in `reportFailure`). Unlike `last-failure.md` (single latest terminal failure, overwritten and removed on completion), this file is cumulative and never auto-removed, so the build loop can be studied and tuned across runs even when a run recovers and finishes cleanly. The revision prompt (`revisePrompt`) is a **surgical repair, not a rewrite**: it names the deliverable, restricts the writer to the edits the findings name (never re-sends the authoring prompt, which biased fresh agents toward regenerating), forbids growing the document >~10% without resolving a blocker/major, and accumulates every verdict across attempts so attempt N sees what N−1 was told; on re-validation the validator is shown the prior verdict and asked to confirm resolved findings before raising new ones (so the loop converges). A revision run is also a gspec-memory **capture run** (record a lesson or state why none generalizes).
- **Content-hash memoization:** a PRD (or spec) that passed QA is recorded with a sha256 of its content in the manifest (`stages.<id>.passed`); on a later pass an unchanged deliverable is skipped rather than re-gated — the features stage restarts over the whole PRD list on every resume, so without this an already-passing PRD burned a fresh budget each time.
- **Writing-phase checkpoint:** the features stage fans out one `feature-writer` per feature and validates only after the whole fan-out completes, so a writer failing (e.g. token exhaustion) pauses the run *before* any memoization. On a re-run (`priorStatus` failed/running) the fan-out therefore skips a feature whose `gspec/features/<slug>.md` already exists and is well-formed (`looksCompletePrd`) — so one failed writer doesn't regenerate every sibling PRD (re-spending its tokens) or overwrite a PRD hand-edited during the pause. A fresh run always writes; the skipped PRD is still re-checked by the validation gate, so a truncated draft that slips the well-formed check is caught there.
- **Delivery, not exit code, decides whether a writer succeeded** (`runWriterResilient`). The exit code answers neither half of the question, and both halves were observed going wrong. *Crash ≠ gate failure:* a writer that exits non-zero is retried once; if it still exits non-zero but its expected artifact is present and well-formed (a transient post-write crash), the driver accepts it and lets QA judge quality rather than restarting the stage. *Exit 0 ≠ delivery:* a writer that exits **0 having produced nothing** — what an engine does when it reports an auth failure, a rate limit, or a usage cap as its final message — is retried once and then fails the stage naming the writer. Previously it was waved through, so the stage marched on and the *validator* failed against a file nobody had written, pausing the run with the wrong culprit. The same insistence covers the driver-written deliverables: the research synthesis must land on disk, and a plan-decomposer's output must actually contain task checkboxes before it is persisted as a plan. A validator that returns no `VERDICT:` line is called out as unvalidated for the same reason.
- **The user's template library reaches an autonomous run.** `gspec-templates` lets a user keep saved specs under `~/.gspec/{stacks,styles,practices,features}/` for a new spec to seed from, and the four matching writers preload the skill. But the convention resolves `~` in the *orchestrating command*, which has a shell — and a build has no such command, while the writers have no `Bash` (their tools are `Read, Write, Edit, Glob, Grep`). So writers were pointed at a folder they had no way to name, and a build ignored every saved template *silently* — indistinguishable from the user having none. The driver is a node process, so it resolves the library itself (`loadTemplateLibrary`, honoring `$HOME`) and lists the matching folder's candidates — absolute path, `name`, `description` — in that stage's brief (`templateNote`), capped at 12 with drops logged. Which one to adopt stays the writer's judgment per the convention. Offered only to a stage's **own** type and only to its **writer** (never a planner, which decides scope rather than content); `profile` and `architecture` have no library by design, being inherently project-specific.
- **Terminal state is machine-readable — a run can no longer end quietly.** The log is prose and the exit code alone was ambiguous (a completed build and a build paused for review both exited 0), so nothing watching a detached run could tell success from a pause, and a hard crash left no mark at all: the manifest stayed at `running`, no `last-failure.md` was written, and the log simply stopped. Three parts:
  - **`.gspec/build/status.json`** — written at every transition, holding `state` (`running` · `complete` · `paused_review` · `failed` · `crashed`), the stage, the reason, the exit code, the engine, and the driving **pid**. `run.json` remains the *control* state the driver resumes from; this is the *answer* to "how did it end?".
  - **A distinct exit code per ending** — `0` complete, `1` failed, `2` paused for spec review, `3` crashed. A watcher branches on a number instead of grepping the log.
  - **Crash handlers** (`uncaughtException`, `unhandledRejection`, `SIGINT`, `SIGTERM`) that stamp `crashed` and write `last-failure.md` **synchronously** — `process.exit` will not wait on a promise. Installed *after* intake, so a Ctrl+C aimed at the interactive interview behaves as it always has, and never for `SIGHUP`, which would defeat `nohup`. A `SIGKILL` outruns any handler, so **`gspec build --status`** treats a `running` record whose pid is gone as a crash, records that, and exits `3`. It falls back to deriving the state from the manifest for runs started before this existed.
- **Bounded specification volume:** nothing bounded how much specification a run produced, and volume became the dominant cost — writers optimize the only axis they are graded on (thoroughness), validators grade *that* on precision (which rewards more text), and every downstream agent then pays to read the result. Three controls, layered:
  - **Section contracts (the primary control).** Each spec type's persona states what a section holds, what it must **not**, and where the excluded content belongs. This is the half of the v1 command templates that was lost when they were compressed into quality bars: positive prescriptions ("a PRD is portable") compress into properties, *negative* ones ("do not read `stack.md`", "no invented Implementation Details section") do not — they are enumerable rules, and dropping them is what let PRDs absorb architecture-tier content (state machines, layout tables) at 60× the section's intended size.
  - **Size budgets.** A per-deliverable word ceiling in `gspec-conventions` → *Size budgets*, preloaded by writers **and** validators, so the checker is not pushing for detail the ceiling forbids. Scaled by a run's **scope tier** (`--scope small|standard|large`, ×0.6/×1/×1.5), which the intake resolves and the manifest pins. A budget is a ceiling, not a target: brevity is never a finding, a missing required section is still a defect, and no fix may grow a spec already at its budget.
  - **Driver-side measurement.** The driver counts each produced spec (`countSpecWords`/`budgetFor`) and reports it against its budget, feeding the overage into the next revision prompt — because a model asked to self-limit reports compliance while writing lines twice as long. The driver's constants are a *copy*; the skill's table is canonical, and `test/build-budgets.test.mjs` fails if they diverge.
  - **Advisory, never a gate.** Over-budget is logged and recorded by QA as a `[minor]` finding, which the severity contract keeps non-blocking. Size alone must never pause a run: the bound belongs in the writer's context, and the measurement exists to make it visible.
- **Cost signal:** each stage records its elapsed time (accumulated across resumes) and the manifest carries a running `totalElapsedMs`, surfaced on the per-stage completion line and the final summary — so a stage that has quietly consumed an hour is visible while the user can still act on it.
- **Per-agent model assignment:** a `models` map in `.gspec/config.json` (project) / `~/.gspec/config.json` (global) assigns a model to each agent, resolved per invocation (`resolveModel`, lib/config.js) most-specific-first — exact agent name > role tier (`writer`/`qa`/`planner`/`implementer`/`researcher`/`inspector`) > `default` — with the project file overriding the global and the resolved string passed to the engine's `--model` (all three adapters accept it via the `model` option). Read fresh each run (not pinned like the engine), so models can be retuned across a `--resume`. This lets cheap stages (validators, foundations) run on a smaller model while architecture/implementation keep the strong one — the main lever on a build's token *cost*.
...
intake(interview)
  ...
  → review        [ HUMAN GATE — pause (exit 2) for spec review; --resume approves; --no-review skips ]
...
- **Implementation verification gate (`implementation-validator`)** — **✅ built (this branch).** The producer≠checker for code (supersedes the vague "code-reviewer" idea). A two-part gate that honors `--no-qa`; this bullet is the as-built contract:
  - **Deterministic part — build + test only** (not the full lint/typecheck/practices pipeline, for now). Multi-module / polyglot aware: one project may ship e.g. a TypeScript frontend + a Java backend, each with its own toolchain and working dir. The **modules table** (name · dir · build · test) lives in **`architecture.md`**, *not* `stack.md` — stack is the tooling *palette* (what *could* build/test; portable, profile-agnostic), architecture is the concrete structure (what *does* exist; technology-aware; already owns Project Structure + Project Setup). The `implementer` generates a committed **`verify.sh`** from that table during scaffolding — fail-fast, prints `FAIL: <module>:<phase>`, hand-editable for setup a command-list can't express (test DB, env, `docker compose`). The **build driver runs `bash verify.sh`** deterministically (exit code = the gate); on failure it re-delegates the `implementer` with the concrete errors — the strongest self-heal loop in the build.
  ...
  - **`gspec-architect` quality bar gains:** a multi-module system must define the modules / build-test table. **`gspec-audit`** catches drift between that table (+ `verify.sh`) and the actual code.
