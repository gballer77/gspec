...
> **Historical record.** Kept in the vocabulary of its time: as of v3.0.0 "deployable" is **module** everywhere (the Deployables table is the **Modules & Verification** table, `deployable:` frontmatter is `module:`). The two-tier layout this proposal introduced still ships unchanged; what each tier *contains* is re-cut by [`gspec-v3-design.md`](./gspec-v3-design.md), which supersedes this document.
