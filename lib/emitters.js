...
// (emitted as a YAML flow list of bare slugs), a `tools` comma string, and an
// optional `model` scalar. name/model are simple slugs emitted unquoted;
// description goes out as a quoted scalar (it contains ": " and "…").
// No `memory:`: memory lives in .gspec/memory/, not a per-agent silo, so the
// manifest's `remembers:` flag drives a skill preload, not a frontmatter field.
...
// OpenCode agent frontmatter. OpenCode has no `skills:` preload; the persona
// (and gspec-memory, where the agent remembers) is inlined into the body
// upstream by composeAgentBody. `tools` is a boolean
