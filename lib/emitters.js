...
// Pi agent frontmatter (the pi-subagents extension — tintinweb/pi-subagents).
// The agent TYPE name comes from the filename, so there's no `name:` field (only
// an optional `display_name`, which we omit — the filename is authoritative).
// `tools` is a CSV allowlist of Pi's lowercase builtin tool ids; `model` is
// omitted so the sub-agent inherits Pi's configured default (Pi model ids are
// provider-specific — we can't assume one). Pi's `skills:` selects skill *scopes*
// (project/user), not a named subset, so the persona is inlined into the body
// upstream rather than preloaded.
const PI_BUILTIN_TOOLS = { read: 'read', write: 'write', edit: 'edit', grep: 'grep', glob: 'glob', bash: 'bash' };
function piToolsCsv(toolsStr) {
  ...
  // builtin equivalent, omit the field (inherit the full parent toolset) rather
  // than emit a partial allowlist that would strip the unmapped capability.
...
export const TARGETS = {
  ...
  pi: {
    ...
    // (tintinweb/pi-subagents), which discovers agent definitions from
    // .pi/agents/<name>.md. gspec's v2 flow delegates to those sub-agents, so
    // the extension is a prerequisite (surfaced at install). Layout:
    ...
    // Pi's `skills:` frontmatter selects skill *scopes* (project/user), not a
    // named subset, so — like opencode/cursor/codex — the persona is inlined
    // into each agent body upstream (see composeAgentBody in build.js).
    preloadsSkills: false,
