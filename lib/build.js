...
import { archLintViolations, designLintViolations, planLintViolations } from '../plugin/hooks/floors/plan-lint.mjs';
...
async function reviseUntilPass(stage, target, ctx, opts) {
  ...
  const judge = async (addressed) => {
    let r = await runAgent(stage.validator, validatorPrompt(stage, validatorTarget, addressed, ctx.scope), ctx);
    let gr = grade(stage, r.text, target);
    if (gr.verdict === null) {
      log(chalk.dim(`      re-running ${stage.validator} — the last response carried no verdict (engine error, not a finding)…`));
      r = await runAgent(stage.validator, validatorPrompt(stage, validatorTarget, addressed, ctx.scope), ctx);
      gr = grade(stage, r.text, target);
    }
    return { v: r, g: gr };
  };
...
async function lintFeatureDesign(cwd, slug) {
  ...
  return designLintViolations(rel, await readOr(cwd, rel), await readOr(cwd, featureFile(slug, FEATURE_FILES.arch)));
}

async function lintFeaturePlan(cwd, slug) {
  const rel = featureFile(slug, FEATURE_FILES.tasks);
  return planLintViolations(rel, await readOr(cwd, rel), await readOr(cwd, featureFile(slug, FEATURE_FILES.arch)));
}
...
async function runImplementScope(stage, scope, basePrompt, ctx) {
  ...
  for (let run = 1; run <= MAX_SCOPE_RUNS; run++) {
    ...
    if (out.code !== 0) {
      ...
      if (errorRetries === 0) {
        errorRetries += 1;
        // Wait before retrying. The observed failure was a 529 Overloaded — a
        // server-side condition that an immediate retry walks straight back
        // into, which is exactly what happened. A short pause costs nothing next
        // to an implement run and turns a guaranteed second failure into a
        // likely recovery.
        log(chalk.dim(`      "${scope.label}": ${stage.agent} exited ${out.code} without checking a task — waiting ${Math.round(TRANSIENT_BACKOFF_MS / 1000)}s, then retrying once (often a transient engine error)…`));
        await sleep(TRANSIENT_BACKOFF_MS);
        continue;
      }
      return out;                                   // errored twice with no progress → surface it
    }
  ...
  switch (stage.type) {
    ...
    case 'implement': {
      ...
      if (stage.orchestrator) {
        ...
        if (!plan) {
          log(chalk.yellow(p.code !== 0
            ? `      ${stage.orchestrator} exited ${p.code} — no wave plan (${tail(String(p.text || '').trim(), 160) || 'no output'})`
            : `      ${stage.orchestrator} answered but its plan was unusable — falling back. This is a parser gap, not an agent failure; its output is in the run log.`));
        }
      }
    ...
    case 'audit': {
      ...
      if (out.code === 0) await recordAuditBaseline(ctx.cwd);
      return { status: out.code === 0 ? 'done' : 'failed', verdict: 'report', reason: out.code === 0 ? undefined : agentFailure(stage.agent, out), detail: out.code === 0 ? undefined : out.text };
    }
