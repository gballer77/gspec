...
import { accumulate, waste, lintVsQa, tallyRules, formatRuleTally } from './usage.js';
...
async function runImplementScope(stage, scope, basePrompt, ctx, { scaffold = false } = {}) {
  ...
  for (let run = 1; run <= MAX_SCOPE_RUNS; run++) {
    ...
    const prompt = `${await promptFor(run, partial)}\n\nThis run is capped at ${IMPLEMENTER_MAX_TURNS} engine turns. Check each task the moment it is complete and verified — never batch the checkboxes — so a fresh run can continue from the plan if the cap is reached. Keep test output terse (a dot or summary reporter; pipe long output through tail).`;
    ...
    if (out.capped) {
      log(chalk.dim(`      "${scope.label}": run ${run} reached the ${IMPLEMENTER_MAX_TURNS}-turn cap${after < before ? ` after checking ${before - after} task(s)` : ' with no task checked'} — continuing on a fresh agent…`));
      out = { ...out, code: 0 };
    ...
    if (after < remaining) {                        // progress → a fresh agent continues
      ...
      if (run < MAX_SCOPE_RUNS && !out.capped) log(chalk.dim(`      "${scope.label}": ${after} task(s) left — continuing on a fresh agent…`));
      continue;
    }
    if (out.code !== 0) {
