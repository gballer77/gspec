...
export const SPEC_BUDGETS = {
  'gspec/profile.md': 2000,
  'gspec/stack.md': 2000,
  'gspec/practices.md': 2000,
  'gspec/style.md': 2000,
  ...
  'gspec/style.html': 1500,
  'gspec/research.md': 2000,
  'gspec/architecture.md': 3000,
  feature: 1800, // gspec/features/<slug>.md
};
...
// when the deliverable carries no word budget (plans are budgeted in tasks).
export function budgetFor(rel, scope = DEFAULT_SCOPE) {
  const base = SPEC_BUDGETS[rel.startsWith('gspec/features/') ? 'feature' : rel];
...
function runIntake(idea, ctx) {
  const prompt = [
    ...
    'large = a multi-deployable system, many integrations, or genuine scale. It scales how much specification gets written, so when in doubt pick the smaller tier.',
...
async function gate(stage, writerPrompt, validatorTarget, ctx, opts = {}) {
  ...
  const outputsOk = sizeTargets.length ? () => outputsPresent(ctx.cwd, sizeTargets) : null;
  ...
  const measure = async () => (await Promise.all(sizeTargets.map((rel) => reportSize(ctx, rel)))).filter(Boolean).join('\n');
  ...
  let v = await runAgent(stage.validator, validatorPrompt(stage, validatorTarget, '', ctx.scope), ctx);
  ...
  for (let r = 1; r <= ctx.qaRetries; r++) {
    ...
    const out = await runWriterResilient(stage.writer, revisePrompt(stage, target, verdicts, sizeNote), ctx, {}, outputsOk);
    if (out.failure) return { status: 'failed', reason: `revision ${r}: ${out.failure}` };
    sizeNote = await measure();
    v = await runAgent(stage.validator, validatorPrompt(stage, validatorTarget, addressed, ctx.scope), ctx);
...
async function runStage(stage, ctx, opts = {}) {
  ...
  switch (stage.type) {
    case 'foundation': {
      ...
      return gate(stage, stageBrief(stage, brief, '', templateNote(stage, ctx.templates)), '', ctx, { revalidate: !!present && rerunning });
    }
    case 'gated': {
    ...
    case 'features': {
      ...
      if (plan) {
        ...
        const runs = await mapLimit(plan, FEATURE_WRITER_CONCURRENCY, async (f) => {
          ...
          const out = await runWriterResilient(stage.writer, stageBrief(stage, brief, assignment, templateNote(stage, ctx.templates)), ctx, {}, () => looksCompletePrd(ctx.cwd, `${f.slug}.md`));
          await reportSize(ctx, `gspec/features/${f.slug}.md`); // advisory (§1)
          return out;
        });
        ...
        const w = await runWriterResilient(stage.writer, stageBrief(stage, brief, `Write the feature PRD for this idea into gspec/features/.${researchNote}`, templateNote(stage, ctx.templates)), ctx, {}, async () => (await listFeaturePrds(ctx.cwd)).length > 0);
    ...
    case 'plan': {
      ...
      for (const prd of prds) {
        ...
        if (!ctx.dryRun) {
          // The driver writes this file from the agent's stdout, so an engine
          // that exited 0 with an error message (or nothing) would otherwise be
          // persisted AS the plan — a real "quiet failure" that only surfaced
          // later, as an implementer with nothing to do.
          const body = extractPlanBody(d.text);
          if (!/^\s*[-*]\s*\[[ xX]\]/m.test(body)) {
            return { status: 'failed', reason: `${stage.writer} returned no usable plan for ${slug} (no task checkboxes in its output — the engine may have errored)`, detail: d.text };
          }
          await mkdir(join(ctx.cwd, 'gspec', 'tasks'), { recursive: true });
          await writeFile(join(ctx.cwd, 'gspec', 'tasks', `${slug}.md`), body, 'utf-8');
        }
        if (!ctx.noQa) {
          const v = await runAgent(stage.validator, validatorPrompt(stage, `gspec/tasks/${slug}.md`, '', ctx.scope), ctx);
    ...
    case 'implement': {
      ...
      let jv = await runAgent(stage.validator, validatorPrompt(stage, jvTarget, '', ctx.scope), ctx, { allowedTools: 'Bash' });
      let jg = grade(stage, jv.text, jvTarget);
      ...
      for (let r = 1; r <= ctx.qaRetries; r++) {
        ...
        jv = await runAgent(stage.validator, validatorPrompt(stage, jvTarget, addressed, ctx.scope), ctx, { allowedTools: 'Bash' });
        jg = grade(stage, jv.text, jvTarget);
        if (jg.verdict === 'PASS') return finishPass(stage, jvTarget, jv, jg, ctx);
      }
