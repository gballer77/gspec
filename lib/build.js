...
async function runImplementScope(stage, scope, basePrompt, ctx, { scaffold = false } = {}) {
  ...
  const promptFor = async (run) => {
    if (run === 1) return firstRunPrompt(basePrompt, scope, await readPlans(ctx.cwd, files), { scaffold });
    const briefs = await remainingTaskBrief(ctx.cwd, files);
    const p = continuationPrompt(stage, ctx.brief, scope, briefs, basePrompt);
  ...
  for (let run = 1; run <= MAX_SCOPE_RUNS; run++) {
    ...
    let after;
    out = await runAgent(stage.agent, await promptFor(run), ctx, {
      allowedTools: 'Bash',
      kind,
