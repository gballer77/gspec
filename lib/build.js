...
import { archLintViolations, designLintViolations, planLintViolations, parallelismViolations, parallelFileOverlapViolations, forwardDepViolations, coversViolations, anchorModules, anchorRefs, originAnchors, duplicateOrigins, containedAnchors, slugifyAnchor } from '../plugin/hooks/floors/plan-lint.mjs';
...
import { accumulate, waste, lintVsQa, tallyRules, formatRuleTally, degradableLintFinding } from './usage.js';
import { parseSleepMinutes, sleepTooShort } from './preflight.js';
import { lintFixPrompt } from './lint-fix.js';
import { remainingTaskBrief, formatRemainingBrief, firstRunGroupBrief, instructionNamesSubset } from './scope-brief.js';
import { partialWorkBrief, partialWorkEvidence } from './partial-work.js';
import { anchoredRevisionBlocks, summarizeRounds, parseFindings, fileAnchor, verdictContractGaps } from './revision-brief.js';
...
import { routesFromArch, primaryFontFamily, renderAvailable, renderEnabled, renderChecks, pickRenderRoot } from '../plugin/hooks/floors/render-lint.mjs';
import { formatAnchorGlossary, formatScreenIds } from './anchor-glossary.js';
import { repairPlan, repairArch, repairDesign } from './auto-repair.js';
...
// Raised from 6 with the turn cap below: a 17-task feature that used to run
// as one agent now finishes in several bounded slices.
const MAX_SCOPE_RUNS = 10;
// The implementer's turn cap per run. Input is turns × a full context window;
// one uncapped feature run was measured at 419 turns and 130M tokens, and
// the context it carried by the end was mostly re-read test output. A capped
// run ends, and the continuation loop briefs a fresh agent on what is left
// (lib/scope-brief.js). Override with GSPEC_IMPLEMENTER_MAX_TURNS.
export const IMPLEMENTER_MAX_TURNS = Number.parseInt(process.env.GSPEC_IMPLEMENTER_MAX_TURNS ?? '120', 10) || 120;
const MAX_STALLS = 2;
...
async function countUnchecked(cwd, files) {
  return (await planCounts(cwd, files)).unchecked;
}

// How many task lines a scope's plan files carry, checked or not, and how
// many are open. The distinction matters: "zero unchecked" means DONE only
// when there were tasks to begin with. A scope whose plan names a file with
// no checkboxes at all — the orchestrator was observed handing a scaffold
// scope `plan: ["gspec/architecture.md"]` — is untracked, not finished, and
// reading it as finished silently skipped the one scope everything else
// depended on. Unreadable files count as nothing, same as before.
async function planCounts(cwd, files) {
  let tasks = 0, unchecked = 0;
  for (const rel of files) {
    let text;
    try { text = await readFile(join(cwd, rel), 'utf-8'); } catch { continue; }
    tasks += (text.match(/^\s*[-*]\s*\[[ xX]\]/gm) || []).length;
    unchecked += (text.match(/^\s*[-*]\s*\[ \]/gm) || []).length;
  }
  return { tasks, unchecked };
}
...
function countNoun(n, noun) { return n === 1 ? `one ${noun}` : `${n} ${plural(noun)}`; }
// "2 fixs" was a real log line. Enough English to cover the nouns this file counts.
...
function reportUsage(manifest) {
  ...
  if (wasted.length) {
    ...
    for (const w of wasted) {
      ...
      const free = w.freeRuns ? `, ${w.freeRuns} of them at ~0 tokens — the engine never answered` : '';
      log(chalk.dim(`    ${w.agent.padEnd(30)} ${String(w.runs).padStart(3)} of ${String(w.totalRuns).padStart(3)} run(s)  in ${k(w.inputTokens).padStart(6)}${kinds ? `  (${kinds}${free})` : free ? `  (${free.slice(2)})` : ''}`));
    }
  }
  ...
  const rules = manifest.lintRules || {};
  if (lq.lint.runs || lq.qa.runs || Object.keys(rules.fired || {}).length) {
    log('');
    ...
    // Which rules, how often. One rule firing across most features in a run
    // means the rule's statement is wrong, not the writers — this line is
    // what makes that visible without reading the log.
    if (Object.keys(rules.fired || {}).length) log(chalk.dim(`    rules fired:    ${formatRuleTally(rules.fired)}`));
    if (Object.keys(rules.autofixed || {}).length) log(chalk.dim(`    auto-repaired:  ${formatRuleTally(rules.autofixed)} (by the driver, no agent run)`));
  }
}
...
function isLintFeedback(agent = '') {
  return /-(lint|autofix)$/.test(String(agent));
}
...
async function runAgent(agentName, prompt, ctx, extra = {}) {
  let out = await ctx.engine.runAgent(agentName, prompt, ctx, {
    ...
    // A turn cap for the runs that need one (the implementer); engines
    // without the flag ignore it.
    maxTurns: extra.maxTurns,
  });
  // A run that used every turn it was allowed is capped whether or not the
  // engine said so in its own words — a CLI that reports the cap differently
  // would otherwise turn it into an "engine error" with three backoff retries.
  if (Number.isInteger(extra.maxTurns) && extra.maxTurns > 0 && !out.capped && typeof out.turns === 'number' && out.turns >= extra.maxTurns) {
    out = { ...out, capped: true };
  }
...
function validatorPrompt(stage, target = '', priorVerdict = '', scope = DEFAULT_SCOPE, note = '') {
  const base = [
...
async function runWriterResilient(agentName, prompt, ctx, extra, artifactOk) {
  ...
  // Continue the draft, don't restart it. A writer that died mid-file used to
  // be re-run from scratch with the identical prompt, re-paying the whole
  // authoring context. If its artifact is on disk but not yet well-formed —
  // which is what "not delivered" means once the file exists — the retry is
  // told to finish it. `extra.artifactPaths` names the deliverable(s); a
  // caller that cannot name them gets the plain retry.
  const partial = await partialArtifacts(ctx.cwd, extra.artifactPaths);
  if (partial.length) log(chalk.dim(`      partial draft found (${partial.join(', ')}) — the retry continues it rather than starting over.`));
  // Whatever the first run was for, the second is either a continuation of a
  // partial draft or the same brief re-sent after an engine error — both are
  // buckets the report exists to show.
  out = await runAgent(agentName, writerRetryPrompt(prompt, partial), ctx, withProgress({ ...extra, kind: partial.length ? 'continuation' : 'transient-retry' }));
  if (out.code === 0 && await delivered()) return { ...out, failure: null };
...
// The retry brief for a writer whose partial draft is on disk: the continue
// instruction first, then the original brief. Unchanged when nothing exists.
export function writerRetryPrompt(prompt, partialPaths = []) {
  if (!partialPaths.length) return prompt;
  const list = partialPaths.length === 1 ? `\`${partialPaths[0]}\`` : partialPaths.map((p) => `\`${p}\``).join(', ');
  return `A partial draft exists at ${list}; read it, keep what is correct, and complete it — do not start over. It was left by a run that was interrupted before finishing, so treat anything missing or malformed in it as unfinished, not as a decision.\n\n${prompt}`;
}

// Which of a writer's deliverables already exist on disk. `paths` is a list
// or an async function returning one (a stage whose deliverable set the
// writer decides resolves it lazily); undefined means the caller cannot say.
async function partialArtifacts(cwd, paths) {
  let list;
  try { list = typeof paths === 'function' ? await paths() : paths; } catch { return []; }
  if (!Array.isArray(list)) return [];
  const out = [];
  for (const rel of list) if (typeof rel === 'string' && await pathExists(cwd, rel)) out.push(rel);
  return out;
}
...
// A FAIL whose findings the driver cannot read — no `- [severity]` items, or
// none with an `anchor:` — triggers the whole-document revision path, which
// was measured at 133–831% of an initial run. Re-asking the VALIDATOR to
// restate the same verdict in the contract shape costs a validator run
// (~100–300k tokens) and is the cheaper of the two by an order of magnitude.
// Once, never re-reviewing: the retry is told to keep its findings.
export function formatRetryPrompt(stage, target, previousVerdict, reason) {
  return [
    `Your verdict for ${target} did not follow the gspec-qa verdict contract: ${reason}.`,
    'Return the SAME verdict and the SAME findings in the exact shape below — do not re-review the document, and do not add, drop, or re-grade a finding. Only the shape changes.',
    '',
    'VERDICT: PASS | FAIL',
    `SPEC: ${target}`,
    'SUMMARY: <2-3 sentences>',
    'FINDINGS:',
    '- [<blocker|major|minor|nit>] <criterion> — <what is wrong>',
    '    evidence: "<quote from the document>"',
    '    anchor: <the exact heading the finding is about, e.g. `### Entity: Order` or `## Data`; for HTML, `line: <n>`>',
    '    fix: <the specific change>',
    '',
    'Every finding carries an `anchor:` line. Your previous verdict, to restate:',
    '',
    previousVerdict,
  ].join('\n');
}
...
async function judgeOnce(stage, target, addressed, ctx, runOpts = {}) {
  let v = await runAgent(stage.validator, validatorPrompt(stage, target, addressed, ctx.scope, runOpts.note), ctx, runOpts);
  let g = grade(stage, v.text, target);
  ({ v, g } = await withContractShape(stage, target, v, g, ctx, runOpts));
  if (g.verdict === null) {
    ...
    for (let attempt = 1; attempt <= MAX_TRANSIENT_RETRIES && g.verdict === null; attempt++) {
      ...
      v = await runAgent(stage.validator, validatorPrompt(stage, target, addressed, ctx.scope, runOpts.note), ctx, runOpts);
...
// The format retry, applied to a graded verdict (see formatRetryPrompt). The
// original is kept whenever the retry is not strictly better: a retry that
// changes the verdict word, or still has no readable finding, is discarded.
async function withContractShape(stage, target, v, g, ctx, runOpts = {}) {
  if (g.verdict !== 'FAIL' || runOpts.noFormatRetry) return { v, g };
  const gaps = verdictContractGaps(v.text);
  if (!gaps.reason) return { v, g };
  log(chalk.dim(`      ${stage.validator}'s verdict ${gaps.reason} — asking it to restate in the contract shape (a validator run, not a writer run)…`));
  const v2 = await runAgent(stage.validator, formatRetryPrompt(stage, target, v.text, gaps.reason), ctx, { ...runOpts, kind: 'format-retry' });
  const g2 = grade(stage, v2.text, target);
  if (g2.verdict === 'FAIL' && !verdictContractGaps(v2.text).reason) return { v: v2, g: g2 };
  log(chalk.dim(`      the restated verdict was not usable (${g2.verdict === null ? 'no verdict' : g2.verdict !== 'FAIL' ? `verdict changed to ${g2.verdict}` : 'still off-contract'}) — keeping the original.`));
  return { v, g };
}
...
export function revisePrompt(stage, target, verdicts, sizeNote = '', docText = '', docs = {}) {
  return revisePromptDetailed(stage, target, verdicts, sizeNote, docText, docs).prompt;
}

// `docs` — other files the findings anchor into (path → text); see
// anchoredRevisionBlocks. `findingsCount` is returned so a caller can tell
// "no anchors" from "no parsable findings at all".
export function revisePromptDetailed(stage, target, verdicts, sizeNote = '', docText = '', docs = {}) {
  const latest = verdicts[verdicts.length - 1] || '';
  const anchored = docText ? anchoredRevisionBlocks(latest, docText, { docs }) : null;
  ...
  const findingsCount = anchored ? anchored.findings.length : parseFindings(latest).length;
  ...
  if (useAnchors) {
    ...
    return { prompt, unanchored, anchored: true, findingsCount };
  }
  ...
  return { prompt, unanchored, anchored: false, findingsCount };
}
...
// Every edit is a full round-trip that re-sends the transcript. A revision
// was measured at 36 turns for one rule-class sweep applied as ~30 small
// edits — ten times the cost of the run that wrote the file.
const EDIT_ECONOMY = 'Apply the edits in as few tool calls as the change allows: when a fix touches more than a handful of places, rewrite the file in ONE write rather than many small edits — every edit is a full round-trip.';
...
async function reviseBrief(ctx, stage, target, verdicts, sizeNote) {
  ...
  // Anchors into other files (an architecture validator anchoring into the
  // module tier): read each named file that exists, so the slice comes from
  // the right document instead of falling back.
  const docs = {};
  for (const f of parseFindings(verdicts[verdicts.length - 1] || '')) {
    const fa = f.anchor ? fileAnchor(f.anchor) : null;
    if (!fa || docs[fa.file] !== undefined || /\s/.test(fa.file)) continue;
    const text = await readOr(ctx.cwd, fa.file, null);
    if (text !== null) docs[fa.file] = text;
  }
  // A stage whose deliverable is a SET (the architecture: system tier plus
  // one file per module) gets the whole set, so an anchor the validator did
  // not prefix with its file — `### Entity: CanonicalQuantity`, which lives
  // in api.md — is still found rather than falling back.
  if (typeof stage.deliverables === 'function') {
    for (const rel of await stage.deliverables(ctx.cwd)) {
      if (rel === target || docs[rel] !== undefined) continue;
      const text = await readOr(ctx.cwd, rel, null);
      if (text !== null) docs[rel] = text;
    }
  }
  const { prompt, unanchored, anchored, findingsCount } = revisePromptDetailed(stage, target, verdicts, sizeNote, doc, docs);
  if (anchored) {
  ...
  } else if (!findingsCount) {
    // A FAIL with nothing the parser can read as a finding: the revision
    // gets the whole verdict and re-reads the whole document. Say so — the
    // validator brief is the thing to fix, and it was silent before.
    log(chalk.yellow(`      ${stage.validator || 'the validator'} returned no finding in the gspec-qa shape ("- [severity] … / anchor:") — the revision falls back to re-reading ${target}.`));
  }
  return prompt;
}
...
export function implementRevisePrompt(stage, scopeLabel, findings, { deterministic = false } = {}) {
  ...
  return [
    ...
    deterministic
      ? 'These are deterministic findings — each names exactly what is wrong. Fix them; do not uncheck a task to make one go away. If a finding says a checked task\'s file does not exist but the work plainly lives under another name, do not duplicate it: create the named file as a thin module that re-exports from where the work is, and say so in your summary.'
...
// `extraGuidance` is a list of lines, or a function of the round's violations
// returning one — a fix that names an unresolved anchor is handed the valid
// anchors, which is cheap to compute and removes the search the writer would
// otherwise do (17 greps across three files, on a measured run).
// `repair`, when given, is tried BEFORE a writer is spent: an async function
// returning `{ repaired: [message] }` after fixing what a regex can fix
// uniquely and losslessly in place (lib/auto-repair.js). The floor then
// re-runs for free; only what is left goes to the writer. One repair pass per
// distinct signature, so a repair that changes nothing cannot loop.
async function lintUntilClean(stage, target, ctx, { lint, outputsOk = null, extraGuidance = [], artifactPaths, repair = null } = {}) {
  const seenFindings = new Set();
  const repairedSignatures = new Set();
  for (let r = 0; ; r++) {
    const violations = await lint();
    if (!violations.length) return null;
    const signature = violations.join('\n');
    if (ctx.recordLintRules) await ctx.recordLintRules('fired', violations);
    if (repair && !repairedSignatures.has(signature)) {
      repairedSignatures.add(signature);
      let fixed = [];
      try { fixed = (await repair(violations))?.repaired || []; } catch { fixed = []; }
      if (fixed.length) {
        log(chalk.dim(`      ${target} — ${countNoun(violations.length, 'lint issue')}; ${countNoun(fixed.length, 'fix')} applied by the driver (no agent run):`));
        for (const f of fixed) log(chalk.dim(`        ⚙ ${f}`));
        await ctx.recordFeedback(stage, `${stage.id}-autofix`, fixed.join('\n'), { target, resolution: 'repaired by the driver — deterministic, no agent run' });
        if (ctx.recordLintRules) await ctx.recordLintRules('autofixed', fixed);
        r -= 1;                                     // a free pass does not spend a round
        continue;
      }
    }
    ...
    const guidance = typeof extraGuidance === 'function' ? await extraGuidance(violations) : extraGuidance;
    const fix = await runWriterResilient(
      stage.writer,
      lintFixPrompt(target, violations, doc, { extraGuidance: guidance }),
      ctx, { kind: 'lint-fix', artifactPaths }, outputsOk,
    );
...
async function gate(stage, writerPrompt, validatorTarget, ctx, opts = {}) {
  ...
  if (opts.revalidate) {
    ...
    const out = await runWriterResilient(stage.writer, writerPrompt, ctx, { artifactPaths: resolveTargets }, outputsOk);
  ...
  if (opts.lint) {
    const failed = await lintUntilClean(stage, target, ctx, { lint: opts.lint, outputsOk, artifactPaths: resolveTargets });
...
async function reviseUntilPass(stage, target, ctx, opts) {
  const { validatorTarget = target, outputsOk = null, measure = async () => '', writerPrompt, artifactPaths } = opts;
  ...
  for (let r = 1; r <= ctx.qaRetries; r++) {
    ...
    const out = await runWriterResilient(stage.writer, await writerPrompt(verdicts, sizeNote), ctx, { kind: 'revision', artifactPaths }, outputsOk);
...
// Run a command and return its stdout, or null on any failure or timeout.
function captureCommand(bin, args, timeoutMs = 5000) {
  return new Promise((resolve) => {
    let p;
    try { p = spawn(bin, args, { stdio: ['ignore', 'pipe', 'ignore'] }); } catch { return resolve(null); }
    let out = '';
    const timer = setTimeout(() => { try { p.kill('SIGKILL'); } catch { /* gone */ } resolve(null); }, timeoutMs);
    p.stdout.on('data', (d) => { out += d; });
    p.on('error', () => { clearTimeout(timer); resolve(null); });
    p.on('close', (code) => { clearTimeout(timer); resolve(code === 0 ? out : null); });
  });
}
...
export async function implementationLint(cwd, { render = true, log: say = () => {} } = {}) {
  ...
  for (const slug of slugs) {
    ...
    if (unresolved.length) {
      // Suffix first; then BASENAME, when exactly one file in the tree carries
      // it. A task said `web/src/recipes/scale-servings.ts` and the file
      // landed at `web/src/recipe/scale-servings.ts`: the work is plainly
      // there, and the implementer — forbidden from editing a checked task —
      // satisfied the finding by creating a duplicate at the named path.
      const real = await walkSourceFiles(cwd);
      for (const r of real) for (const named of unresolved) if (r.endsWith(`/${named}`)) present.add(named);
      const byBase = new Map();
      for (const r of real) { const b = r.split('/').pop(); (byBase.get(b) || byBase.set(b, []).get(b)).push(r); }
      const realSet = new Set(real);
      for (const named of unresolved) {
        if (present.has(named)) continue;
        const hits = byBase.get(named.split('/').pop()) || [];
        if (hits.length === 1) { present.add(named); say(chalk.dim(`      note: ${named} resolved to ${hits[0]} (same file name, different directory)`)); continue; }
        // Same directory, a dotted prefix dropped: the plan wrote
        // `resources/recipes/recipes.routes.ts` and the implementer wrote
        // `resources/recipes/routes.ts`. The work is there; the name is not.
        const dir = named.includes('/') ? named.slice(0, named.lastIndexOf('/')) : '';
        const base = named.split('/').pop();
        const parts = base.split('.');
        for (let i = 1; i < parts.length - 1; i++) {
          const candidate = `${dir ? `${dir}/` : ''}${parts.slice(i).join('.')}`;
          const hit = [...realSet].find((r) => r === candidate || r.endsWith(`/${candidate}`));
          if (hit) { present.add(named); say(chalk.dim(`      note: ${named} resolved to ${hit} (same directory, shorter name)`)); break; }
        }
      }
    }
...
async function renderLint(cwd, slugs, say) {
  // A multi-module project has no root package.json — the web app lives in
  // a Modules-table dir. Candidates are the root and every module dir; the
  // first that can be rendered (Playwright + a dev/start script) serves.
  const candidates = [{ dir: '.', pkgJsonText: await readOr(cwd, 'package.json', null) }];
  for (const m of parseModulesTable(await readOr(cwd, 'gspec/architecture.md'))) {
    if (m.dir && m.dir !== '.') candidates.push({ dir: m.dir, pkgJsonText: await readOr(cwd, join(m.dir, 'package.json'), null) });
  }
  const root = pickRenderRoot(candidates);
  if (!root) return [];                             // silent: the common case, and not a gap in the project
  ...
  for (const slug of slugs) {
    ...
    const r = await renderChecks(join(cwd, root.dir), routes, { fontFamily }, { pkgJsonText: root.pkgJsonText, screensDir: join(cwd, SCREENS_DIR, slug) });
    if (r.skipped) {
      say(chalk.dim(`      render check skipped for "${slug}" — ${r.skipped}`));
      // A server that would not come up for one feature will not for the
      // next; waiting the full timeout per feature cost three minutes a round.
      if (/did not answer|could not launch|could not be loaded/.test(r.skipped)) { say(chalk.dim('      render check skipped for the remaining features this round (same server).')); break; }
      continue;
    }
    say(chalk.dim(`      rendered ${countNoun(routes.length, 'route')} for "${slug}" from ${root.dir === '.' ? 'the project root' : root.dir}${r.screens.length ? ` — screenshots in ${join(SCREENS_DIR, slug)}` : ''}${r.violations.length ? ` — ${countNoun(r.violations.length, 'issue')}` : ''}`));
...
export async function splitScopesByFeature(cwd, wave) {
  ...
  for (const scope of wave) {
    // A scope with no `plan` is untracked: one uncapped run, no continuation,
    // no task-group split, and "built" whether or not a box was checked. An
    // orchestrator omitted the field on every scope of a resumed run while
    // naming the plan file in each instruction — so when exactly one feature
    // is named, the driver fills it in and says so. A scaffold names none.
    const inferred = !(scope.plan || []).length ? await inferScopePlan(cwd, scope) : null;
    if (inferred) log(chalk.dim(`      note: scope "${scope.label}" named no plan file — inferred ${inferred} from its instruction`));
    ...
    const files = [];
    for (const p of inferred ? [inferred] : (scope.plan || [])) {
...
// The one plan file a plan-less scope is about, or null. Read from a
// `gspec/features/<slug>/tasks.md` path in the instruction or label, else
// from a known feature slug at the start of the label ("pantry-and-shopping
// (T1-T12)"). Exactly one feature, and its plan must exist on disk.
export async function inferScopePlan(cwd, scope) {
  // A scaffold names no feature of its own, but its instruction may well
  // mention one ("the importer scaffolds itself later via recipe-import-cli
  // T1 — do not create it here"). Seen live: the scaffold was handed the
  // importer's plan and tracked against it. A scaffold stays untracked.
  if (/scaffold/i.test(String(scope.label || ''))) return null;
  const text = `${scope.label || ''}\n${scope.instruction || ''}`;
  const slugs = new Set();
  for (const m of text.matchAll(/gspec\/features\/([a-z0-9][a-z0-9-]*)\/tasks\.md/g)) slugs.add(m[1]);
  if (!slugs.size) {
    const known = await listFeatureSlugs(cwd);
    const head = String(scope.label || '').trim().toLowerCase();
    for (const slug of known) if (head === slug || head.startsWith(`${slug} `) || head.startsWith(`${slug}:`) || head.startsWith(`${slug}(`)) slugs.add(slug);
  }
  if (slugs.size !== 1) return null;
  const rel = await planPath(cwd, [...slugs][0]);
  return (await pathExists(cwd, rel)) ? rel : null;
}
...
async function lintFeaturePlan(cwd, slug) {
  ...
  return [
    ...
    ...parallelFileOverlapViolations(rel, tasks),
    ...forwardDepViolations(rel, tasks),
...
// Both prompts hand over the exact identifiers the floors will check for
// (lib/anchor-glossary.js), so the writer copies rather than derives them.
async function featureDesignPrompt(ctx, slug, rel) {
  const archRel = featureFile(slug, FEATURE_FILES.arch);
  return [
    `Write the feature design for "${slug}" to ${rel} (use this exact path) — a self-contained, renderable HTML mockup.`,
    `Its architecture is ${archRel} and its PRD is ${await prdPath(ctx.cwd, slug)}.`,
    'One `<section id="screen-<kebab>">` per `### Screen:` in the architecture\'s ## UI section, matching in both directions.',
    formatScreenIds(await readOr(ctx.cwd, archRel), archRel),
...
async function featurePlanPrompt(ctx, slug, rel) {
  ...
  return [
    ...
    `Its PRD is ${await prdPath(ctx.cwd, slug)} and its architecture is ${archRel}.`,
    ...
    formatAnchorGlossary(await readOr(ctx.cwd, archRel), archRel),
    'Preserve every checked task verbatim; append new ones with the next free ID.',
  ].filter(Boolean).join('\n');
}

// Apply the deterministic repairs (lib/auto-repair.js) to one feature file
// and write it back when anything changed. Returns { repaired: [message] }.
async function autoRepairFeature(cwd, slug, file) {
  const rel = featureFile(slug, file);
  const text = await readOr(cwd, rel, null);
  if (text === null) return { repaired: [] };
  const arch = await readOr(cwd, featureFile(slug, FEATURE_FILES.arch));
  let result;
  if (file === FEATURE_FILES.tasks) result = repairPlan(text, arch);
  else if (file === FEATURE_FILES.arch) result = repairArch(text);
  else if (file === FEATURE_FILES.design) result = repairDesign(text, arch);
  else return { repaired: [] };
  if (result.repairs.length && result.text !== text && !skipWrite()) await writeFile(join(cwd, rel), result.text, 'utf-8');
  return { repaired: result.text !== text ? result.repairs : [] };
}

// The identifiers a lint fix needs beside an unresolved reference: the valid
// anchors (plan) or screen ids (design) for this feature. Computed per
// feature, per round; static guidance is concatenated with it.
async function lintGlossary(ctx, stage, slug) {
  const archRel = featureFile(slug, FEATURE_FILES.arch);
  const arch = await readOr(ctx.cwd, archRel);
  if (stage.file === FEATURE_FILES.tasks) return formatAnchorGlossary(arch, archRel);
  if (stage.file === FEATURE_FILES.design) return formatScreenIds(arch, archRel);
  return '';
}
  ...
  for (const slug of targets) {
    ...
    if (stage.lint) {
      const failed = await lintUntilClean(stage, target, ctx, {
        ...
        // The rename rule always; the valid identifiers only when a violation
        // is about one that did not resolve.
        extraGuidance: async (violations) => [
          ...ANCHOR_RENAME_GUIDANCE,
          ...(violations.some((v) => /does not resolve|has no matching|no <section id=/.test(v)) ? ['', await lintGlossary(ctx, stage, slug)].filter(Boolean) : []),
        ],
        artifactPaths: [target],
        repair: () => autoRepairFeature(ctx.cwd, slug, stage.file),
      });
      if (failed) return failed;
    }
...
export function firstRunPrompt(basePrompt, scope, plans = [], { scaffold = false } = {}) {
  ...
  // "Tasks T1–T17" that names every unchecked task is the whole feature, not
  // a granularity decision — the orchestrator writes the range by habit. Only
  // a proper subset of the plan is respected as-is.
  if (instructionNamesSubset(scope.instruction, plans[0].text)) return basePrompt;
  const group = firstRunGroupBrief(plans[0].rel, plans[0].text);
  // FIRST, not appended: an implementer given "implement the feature end to
  // end, tasks T1–T25" and then, 800 words later, "group 1 only — then STOP",
  // built the whole feature and was cut by the turn cap with nothing checked.
  return group ? `${group}\n\nThe scope instruction below describes the WHOLE feature for context. This run owns only the group above; do not start any other task.\n\n${basePrompt}` : basePrompt;
}
...
async function runImplementScope(stage, scope, basePrompt, ctx, { scaffold = false } = {}) {
  ...
  const promptFor = async (run, partial = '') => {
    ...
    if (run === 1) p = firstRunPrompt(basePrompt, scope, await readPlans(ctx.cwd, files), { scaffold });
      ...
      // What the run is briefed with, in the terms that matter: the tasks
      // left and whether the spec text it needs is carried inline (so it has
      // no spec file to open) or only named. Word counts against the base
      // brief said "no shrink" on every continuation of a 500-word brief —
      // the saving was never in the prompt's length.
      const left = briefs.reduce((n, b) => n + b.tasks.length, 0);
      const blocks = briefs.reduce((n, b) => n + (b.sectionTexts?.length || 0), 0);
      const named = briefs.reduce((n, b) => n + b.sections.length, 0);
      log(chalk.dim(`      "${scope.label}": continuation brief — ${countNoun(left, 'task')} left, ${blocks ? `${countNoun(blocks, 'spec block')} inlined (no spec file to open)` : named ? `${countNoun(named, 'section')} named (too much to inline)` : 'no anchors cited'}; ${countSpecWords(p)} words`));
    }
    return partial ? `${p}\n\n${partial}` : p;
  };
  ...
  // A plan with no tasks in it tracks nothing. Run the scope once, the way a
  // scaffold with no plan file runs — never skip it as "already checked".
  const counts = await planCounts(ctx.cwd, files);
  if (!counts.tasks) {
    log(chalk.dim(`      "${scope.label}": its plan file(s) carry no task checkboxes (${files.join(', ')}) — untracked, running once.`));
    return runAgent(stage.agent, await promptFor(1), ctx, { allowedTools: 'Bash' });
  }

  let out, stalls = 0, errorRetries = 0;
  let remaining = counts.unchecked;
  // The turn cap adapts: a capped run that checked NOTHING may simply hold one
  // task bigger than the cap (a migration, a scaffold-like foundation), and
  // counting that as a stall ended the scope after two tries. The cap doubles
  // for the next run, up to three times the base, before a stall is counted.
  let cap = IMPLEMENTER_MAX_TURNS;
  const capCeiling = IMPLEMENTER_MAX_TURNS * 3;
  let raised = false;
  // Slices scale with the plan: ten was the whole allowance for a 25-task
  // feature landing two tasks a slice.
  const maxRuns = Math.max(MAX_SCOPE_RUNS, Math.ceil(counts.unchecked / 2));
  ...
  for (let run = 1; run <= maxRuns; run++) {
    ...
    const prompt = `${await promptFor(run, partial)}\n\nThis run is capped at ${cap} engine turns. Check each task the moment it is complete and verified — never batch the checkboxes — so a fresh run can continue from the plan if the cap is reached. Keep test output terse (a dot or summary reporter; pipe long output through tail).`;
    ...
    // A run stopped by the cap is not an error: whatever it checked stands,
    // and the next run picks up the rest. Normalize the exit so the gates
    // judge the work rather than the driver reporting "exited 1".
    raised = false;
    if (out.capped) {
      log(chalk.dim(`      "${scope.label}": run ${run} reached the ${cap}-turn cap${after < before ? ` after checking ${before - after} task(s)` : ' with no task checked'} — continuing on a fresh agent…`));
      out = { ...out, code: 0 };
      if (after >= before && cap < capCeiling) {
        cap = Math.min(cap * 2, capCeiling);
        raised = true;
        log(chalk.dim(`      "${scope.label}": nothing checked under the cap — raising it to ${cap} turns for the next run before counting a stall.`));
      }
      // A capped run that checked nothing has usually WRITTEN plenty — one was
      // measured at 24 files and 35M tokens with every box still open. The
      // next run must know, or it starts over; same evidence as a retry.
      if (after >= before) {
        try {
          partial = partialWorkBrief(await partialWorkEvidence(ctx.cwd, files, { gitStatus: (cwd) => gitCapture(cwd, ['status', '--porcelain']) }));
          if (partial) log(chalk.dim(`      "${scope.label}": the capped run left work on disk — the next run is told to verify and check it before anything else.`));
        } catch { partial = ''; }
      }
    }
    ...
    if (after < remaining) {                        // progress → a fresh agent continues
      ...
      if (run < maxRuns && !out.capped) log(chalk.dim(`      "${scope.label}": ${after} task(s) left — continuing on a fresh agent…`));
      continue;
    }
    if (out.code !== 0) {
    ...
    if (raised) continue;                           // a raised cap is a retry with more room, not a stall
  ...
  switch (stage.type) {
    ...
    case 'features': {
      ...
      if (plan) {
        ...
        const runs = await mapLimit(plan, FEATURE_WRITER_CONCURRENCY, async (f) => {
          ...
          const out = await runWriterResilient(stage.writer, stageBrief(stage, brief, assignment, templateNote(stage, ctx.templates)), ctx, { artifactPaths: () => prdPath(ctx.cwd, f.slug).then((p) => [p]) }, () => looksCompletePrd(ctx.cwd, f.slug));
      ...
      for (const prd of slugs) {
        ...
        let g = grade(stage, v.text, target);
        ({ v, g } = await withContractShape(stage, target, v, g, ctx));
        if (g.verdict === 'FAIL') {
          ...
          for (let r = 1; r <= ctx.qaRetries; r++) {
            ...
            const rev = await runWriterResilient(stage.writer, await reviseBrief(ctx, stage, target, verdicts, sizeNote), ctx, { kind: 'revision', artifactPaths: [target] }, () => looksCompletePrd(ctx.cwd, prd));
    ...
    case 'implement': {
      ...
      for (let r = 0; ; r++) {
        ...
        const stuck = seenFindings.has(signature);
        // A finding the implementer was already sent and could not satisfy —
        // a checked task's file under a name it may not change, a console
        // error from a library — while verify.sh is green is not a build to
        // fail; it is a question for the validator. Font and HTTP-status
        // findings stay blocking: those the implementer can always fix.
        if (stuck && verifyGreen && findings.every(degradableLintFinding)) {
          log(chalk.yellow(`      ${countNoun(findings.length, 'implementation lint issue')} repeated after a fix with verify.sh green — the implementer cannot satisfy them mechanically; passing them to the validator as advisory rather than failing the stage:`));
          for (const f of findings) log(chalk.dim(`        · ${f}`));
          await ctx.recordAdvisory(stage, 'the implementation', `Deterministic findings left unresolved after a repeated fix round (verify.sh green) — judged by the validator instead:\n${findings.map((f) => `- ${f}`).join('\n')}`);
          advisoryLint = findings;
          break;
        }
        if (r >= MAX_LINT_ROUNDS || stuck) {
          return {
        ...
        if (ctx.recordLintRules) await ctx.recordLintRules('fired', findings);
      ...
      const jvOpts = {
        allowedTools: 'Bash',
        note: advisoryLint.length
          ? `The deterministic lint left these findings unresolved after the implementer's fix round, with verify.sh green. Judge each on its merits — a checked task whose file lives under another name is not missing work; a library's console error is not a defect in this code — and raise only what is real:\n${advisoryLint.map((f) => `- ${f}`).join('\n')}`
          : undefined,
      };
...
export async function runBuild({ idea, cwd = process.cwd(), engine, noQa = false, noReview = false, research = false, scope, qaRetries, resume = false, dryRun = false, permissionMode = 'acceptEdits', piPermissionLevel, parallel, notify: notifyFlag } = {}) {
  ...
  // Per-rule tallies for the Lint vs QA report (lib/usage.js): every
  // violation a floor raised, and every repair the driver applied.
  ctx.recordLintRules = async (kind, messages) => {
    if (dryRun || !messages?.length) return;
    tallyRules(((manifest.lintRules ??= {})[kind] ??= {}), messages);
    await saveManifest(cwd, manifest);
  };
  ...
  // A Mac that sleeps between engine turns stalls the build for as long as it
  // sleeps, and nothing in the run says so (lib/preflight.js). Warn; never block.
  if (!dryRun && process.platform === 'darwin') {
    const minutes = parseSleepMinutes(await captureCommand('pmset', ['-g'], 3000));
    if (sleepTooShort(minutes)) {
      log(chalk.yellow(`  Note: this Mac sleeps after ${countNoun(minutes, 'minute')} idle, and a build stalls while it sleeps. Run under caffeinate (\`caffeinate -i gspec build …\`) or raise the limit (\`sudo pmset -a sleep 0\`).`));
    }
  }
