...
function reportPauses(manifest) {
  ...
  for (const p of pauses) {
    ...
    log(chalk.dim(`    · ${p.stage} — ${p.limit ? 'usage limit' : 'failure'}, ${how}`));
  }
}
...
function unvalidatedReason(stage, target, text, after = '') {
  ...
  return `${target} could not be validated${after ? when : ''} — ${stage.validator} returned no verdict twice (an engine error, not a finding)`;
}
...
async function runStage(stage, ctx, opts = {}) {
  ...
  switch (stage.type) {
      ...
      if (stage.orchestrator) {
        ...
        if (!plan) {
          if (p.code !== 0) {
            ...
            if (!skipWrite()) await writeFile(join(ctx.cwd, UNPARSED_PLAN_PATH), [
    ...
    case 'audit': {
      ...
      if (report) {
        if (!skipWrite()) await writeFile(join(ctx.cwd, AUDIT_REPORT_PATH), `# Reconcile audit\n\n${report}\n`, 'utf-8');
