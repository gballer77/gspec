...
function reportPauses(manifest) {
  ...
  for (const p of pauses) {
    ...
    const why = p.limit ? 'usage limit' : p.unattended ? 'not running' : 'failure';
    ...
    if (open) {
      ...
      if (gapMs > STOPPED_GAP_MS) {
        ...
        (manifest.pauses ??= []).push({
          ...
          reason: 'the run was not running (the driver was stopped, not paused)',
          limit: false,
          unattended: true,
        });
        await saveManifest(cwd, manifest);
        console.log(chalk.dim(`  Continuing a run that was not running for ${humanMs(gapMs)} (stopped at "${stage}", no failure recorded).`));
      }
    }
  }
