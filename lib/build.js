...
import { archLintViolations, designLintViolations, planLintViolations, parallelismViolations, coversViolations, anchorModules, originAnchors, duplicateOrigins, containedAnchors, slugifyAnchor } from '../plugin/hooks/floors/plan-lint.mjs';
...
async function runAgent(agentName, prompt, ctx, extra = {}) {
  ...
  if (ctx.recordUsage) await ctx.recordUsage(agentName, out, extra.kind || 'initial');
  return out;
}
...
export function revisePrompt(stage, target, verdicts, sizeNote = '') {
  ...
  return [
    ...
    'This failed verdict is corrective feedback: if the gspec-memory convention is in your instructions, record the generalizable memory to .gspec/memory/pending/ (or state in your summary why the finding was purely project-specific) before returning.',
    '',
    history,
  ].join('\n');
}
...
async function reviseUntilPass(stage, target, ctx, opts) {
  ...
  for (let r = 1; r <= ctx.qaRetries; r++) {
    ...
    const out = await runWriterResilient(stage.writer, writerPrompt(verdicts, sizeNote), ctx, { kind: 'revision' }, outputsOk);
  ...
  for (const slug of targets) {
    ...
    if (stage.lint) {
      ...
      for (let r = 0; ; r++) {
        ...
        await ctx.recordFeedback(stage, `${stage.id}-lint`, violations.join('\n'), { target, attempt: r + 1 });
        const fix = await runWriterResilient(
          ...
          [`Fix these mechanical problems in ${target}. They are deterministic — each names exactly what is wrong.`,
            ...
            'Change nothing else.',
            '',
            // A fix that RENAMES an anchor is the one move that can trade one
            // violation for a different one, and the budget only covers a
            // single round. Observed end to end: a writer qualified an anchor
            // ("### Entity: SimEvent (wave variants)"), the grammar floor
            // rejected the name, the writer renamed it to the canonical
            // "### Entity: SimEvent" — correct, and the only possible name —
            // then wrote `defined-in:` because nothing in that finding said
            // otherwise. That created a second origin, which failed the stage
            // on the re-lint with the retry already spent. The rename is not
            // the mistake; being told about it one violation at a time is.
            ...
            'one-round fix replaces one violation with another and fails the stage.'].join('\n'),
      ...
      for (const prd of slugs) {
        ...
        if (g.verdict === 'FAIL') {
          ...
          for (let r = 1; r <= ctx.qaRetries; r++) {
            ...
            const rev = await runWriterResilient(stage.writer, revisePrompt(stage, target, verdicts, sizeNote), ctx, { kind: 'revision' }, () => looksCompletePrd(ctx.cwd, prd));
      ...
      if (plan) {
        ...
        for (let i = 0; i < plan.length; i++) {
          ...
          const runs = await Promise.all(wave.map(async (scope) => {
            ...
            const r = await runImplementScope(stage, scope, basePrompt, ctx);
...
export async function runBuild({ idea, cwd = process.cwd(), engine, noQa = false, noReview = false, research = false, scope, qaRetries, resume = false, dryRun = false, permissionMode = 'acceptEdits', piPermissionLevel } = {}) {
  ...
  ctx.recordUsage = async (agentName, out, kind = 'initial') => {
    const u = out?.usage;
    if (!u || dryRun) return;
    ...
    add(a);
    // Same numbers, split by whether this run was authoring or repairing. Kept
    // as a sub-bucket rather than a separate agent key so the existing totals
    // and their ordering are untouched.
    add((a.byKind ??= {})[kind] ??= { runs: 0, in: 0, cacheRead: 0, cacheWrite: 0, out: 0, turns: 0, costUsd: 0 });
    await saveManifest(cwd, manifest);
  };
