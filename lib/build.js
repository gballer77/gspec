...
export async function runBuild({ idea, cwd = process.cwd(), engine, noQa = false, noReview = false, qaRetries, resume = false, dryRun = false, permissionMode = 'acceptEdits', piPermissionLevel } = {}) {
  ...
  if (!manifest) {
    manifest = initManifest(idea, { engine: engineName, noQa, noReview, qaRetries: explicitQaRetries, permissionMode });
  ...
  const ctx = { cwd, engine: selectedEngine, noQa: manifest.noQa, qaRetries: qaRetryCount, permissionMode: manifest.permissionMode, piPermissionLevel, dryRun, brief: '', log };
  ...
  log(chalk.dim(`  engine: ${engineName} · QA gates: ${qaMode} · spec review: ${skipReview ? 'off (--no-review)' : 'pauses before implementation'} · resumable · ${dryRun ? 'DRY RUN' : 'autonomous after intake'}\n`));
