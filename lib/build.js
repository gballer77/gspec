...
export async function runBuild({ idea, cwd = process.cwd(), engine, noQa = false, noReview = false, research = false, scope, qaRetries, resume = false, dryRun = false, permissionMode = 'acceptEdits', piPermissionLevel, parallel } = {}) {
  ...
  const ctx = { cwd, engine: selectedEngine, noQa: manifest.noQa, research: !!manifest.research, qaRetries: qaRetryCount, scope: scope ?? manifest.scope ?? DEFAULT_SCOPE, permissionMode: manifest.permissionMode, piPermissionLevel, dryRun, brief: '', templates, log, parallel: parallel ?? 'auto' };
