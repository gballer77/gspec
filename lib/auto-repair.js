...
export function repairPlan(tasksText, archText) {
  ...
  // Pass 4: two [P] tasks naming the same file — the lower-numbered keeps the
  // marker, the others lose it (serial is never wrong). Uses the task's own
  // block text, so a file named on a `- Files:` line counts.
  const blockText = (t) => { let end = t.line + 1; while (end < lines.length && !TASK_LINE.test(lines[end])) end++; return lines.slice(t.line, end).join('\n'); };
  const byFile = new Map();
  for (const t of tasks) {
    if (t.checked || !t.parallel) continue;
    for (const f of pathsNamedBy(blockText(t))) (byFile.get(f) || byFile.set(f, []).get(f)).push(t);
  }
  for (const [file, group] of byFile) {
    ...
    for (const t of group.slice(1)) {
      ...
      lines[t.line] = lines[t.line].replace(/(\*\*T\d+\*\*)\s*\[P\]\s?/, '$1 ');
      t.parallel = false;
      repairs.push(`T${t.id}: dropped [P] — it writes \`${file}\`, which [P] task T${group[0].id} also writes; running it after is always safe`);
    }
  }
