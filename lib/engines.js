...
export const ENGINES = {
  ...
  codex: {
    ...
    async runAgent(agentName, prompt, ctx, { needsBash } = {}) {
      ...
      const instructions = await readInjectedAgent(ctx.cwd, join('.codex', 'agents', `${agentName}.toml`), codexInstructions);
  ...
  pi: {
    ...
    async runAgent(agentName, prompt, ctx, { needsBash } = {}) { // eslint-disable-line no-unused-vars
      ...
      const instructions = await readInjectedAgent(ctx.cwd, join('.pi', 'agents', `${agentName}.md`), stripFrontmatter);
      const full = inject(instructions, prompt);
