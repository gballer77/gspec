  ...
  try {
    ...
    if (o && typeof o === 'object' && ('result' in o || 'usage' in o)) {
      return {
        ...
        // `error_max_turns`: the run was stopped by --max-turns, not by a
        // fault. The driver continues it on a fresh agent rather than
        // treating the exit as an engine error.
        capped: o.subtype === 'error_max_turns',
      };
    }
...
export const ENGINES = {
  claude: {
    ...
    async runAgent(agentName, prompt, ctx, { needsBash, needsWeb, model, maxTurns } = {}) {
      ...
      // A bounded run. Cost is turns × a full context window, and an
      // implementer left to run a whole feature was measured at 419 turns and
      // 130M tokens; a capped run ends and the driver continues it small.
