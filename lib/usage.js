...
//       format-retry     — a validator asked to restate an off-contract verdict
...
export const USAGE_KINDS = ['initial', 'revision', 'lint-fix', 'continuation', 'transient-retry', 'format-retry'];
...
export function accumulate(acc, agentName, out, kind = 'initial', flags = {}) {
  ...
  if (flags.progress === false) {
    add(a.noProgress ??= empty());
    // A run the engine never really answered — a network blip, a limit —
    // reads nothing. It is still a run that made no progress, but it is not
    // the waste the report is about, and lumping it in overstated the count.
    const read = (u.input_tokens || 0) + (u.cache_read_input_tokens || 0) + (u.cache_creation_input_tokens || 0);
    if (read < FREE_RUN_TOKENS) a.noProgressFree = (a.noProgressFree || 0) + 1;
  }
  return acc;
}

// Below this many input tokens a run did not do anything — it failed before
// it started (an unreachable engine, a limit hit on the first call).
export const FREE_RUN_TOKENS = 5000;
...
const RULE_KEYS = [
  ...
  // Before honesty: a file-overlap repair also says "dropped [P]".
  ['[P] file overlap', /both \[P\] and both write|also writes; running it after/],
  ['[P] honesty', /marked \[P\]|dropped \[P\]/],
  ['forward dep', /comes? later in the plan/],
...
/**
 * A floor finding the implementer may be unable to satisfy however hard it
 * tries: a checked task's file named differently from what was written (the
 * task is immutable), or a console error raised by a library. When such a
 * finding repeats after a fix round while verify.sh is green, the gate hands
 * it to the validator as advisory instead of failing the build. A font that
 * fell back or a route that 404s is always fixable, so those never degrade.
 */
export function degradableLintFinding(message) {
  const key = lintRuleKey(message);
  if (key === 'missing work') return true;
  if (key === 'render' && /console error/.test(String(message))) return true;
  return false;
}
