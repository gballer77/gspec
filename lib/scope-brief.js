...
import { sliceSections } from './revision-brief.js';

// Above this many words of inlined spec text the brief names sections instead
// of carrying them — a prompt is not the place for a whole architecture.
export const INLINE_BUDGET_WORDS = 6000;
 ...
 */
export function remainingBriefFor({ tasksRel, tasksText, archRel, archText = '', moduleFileExists = () => true, moduleTexts = {} }) {
  ...
  const sections = [];
  const sectionTexts = [];   // { rel, heading, text } — the cited blocks, verbatim
  const files = new Set();
  for (const [slug, entry] of index) {
    if (!cited.has(looseSlug(slug))) continue;
    sections.push(entry.heading);
    // An agent reads files, not sections: told "read these sections only" a
    // continuation opened arch.md, design.html, prd.md and both module tiers
    // whole (measured: ~14 spec reads per continuation). So the brief carries
    // the block itself, and says there is nothing to open.
    const [slice] = sliceSections(archText, [entry.heading]);
    if (slice.found) sectionTexts.push({ rel: archRel, heading: entry.heading, text: slice.text });
    if (entry.module) {
  ...
  // The same anchors as they appear in the module tier — a `uses:` stub in
  // the feature points at a full definition there — inlined from each listed
  // module file when its text was supplied.
  for (const rel of files) {
    const text = moduleTexts[rel];
    if (!text) continue;
    for (const line of String(text).split('\n')) {
      if (!/^###\s/.test(line)) continue;
      const heading = line.trim();
      if (!cited.has(looseSlug(slugifyAnchor(heading)))) continue;
      const [slice] = sliceSections(text, [heading]);
      if (slice.found) sectionTexts.push({ rel, heading, text: slice.text });
    }
  }
  const inlinedWords = sectionTexts.reduce((n, x) => n + x.text.split(/\s+/).length, 0);
  ...
  return {
    ...
    sections,
    sectionTexts: inlinedWords <= INLINE_BUDGET_WORDS ? sectionTexts : [],
    inlinedWords,
    files: [...files],
...
export function formatRemainingBrief(briefs) {
  ...
  for (const b of list) {
    ...
    if (b.sectionTexts?.length) {
      inlined = true;
      lines.push('The spec blocks these tasks cite, verbatim — this is the architecture you need; do not open the spec files:');
      for (const x of b.sectionTexts) lines.push(`--- ${x.rel} → ${x.heading} ---`, x.text, '');
    } else {
  ...
  lines.push(inlined
    ? 'Everything else in the feature folder and the architecture is done or not needed for these tasks. Open only the SOURCE files a remaining task names (and the plan file, to check boxes) — never the spec files; their relevant blocks are above.'
    : 'Everything else in the feature folder and the architecture is done or not needed for these tasks — do not re-read it. Open other files only when a remaining task names them.');
  return lines.join('\n');
}
...
export async function remainingTaskBrief(cwd, planFiles = []) {
  ...
  for (const tasksRel of planFiles) {
    ...
    // Resolve existence up front so the core stays synchronous, and read the
    // module files that exist so their cited blocks can be inlined.
    ...
    for (const f of draft.files) {
      ...
      present.add(f);
      try { moduleTexts[f] = await readFile(join(cwd, f), 'utf-8'); } catch { /* listed, not inlined */ }
    }
    out.push(remainingBriefFor({ tasksRel: rel, tasksText, archRel, archText, moduleFileExists: (p) => present.has(p), moduleTexts }));
  }
  return out;
}
...
export function firstRunGroupBrief(tasksRel, tasksText, size = TASK_GROUP_THRESHOLD) {
  ...
  return [
    `THIS RUN: group 1 of ${groups.length} of a ${byId.size}-task plan — ONLY the tasks listed below, in order. Check each task's box the moment it is complete and verified, and when the last one is checked, return immediately. A fresh run continues with the next group (${rest.flat().join(', ')}); anything you build beyond this group is not credited to this run, and a run that ends with no box checked loses its work.`,
    `Group 1 (${tasksRel}):`,
