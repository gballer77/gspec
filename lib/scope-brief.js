...
import { TASK_FIELD, normalizeAnchorRef, slugifyAnchor, anchorModules, anchorRefs } from '../plugin/hooks/floors/plan-lint.mjs';
...
export function remainingBriefFor({ tasksRel, tasksText, archRel, archText = '', moduleFileExists = () => true }) {
  ...
  const cited = new Set(remaining.flatMap((t) => t.anchors));
  const sections = [];
  const files = new Set();
