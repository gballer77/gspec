...
export function parseFindings(verdictText) {
  ...
  for (const raw of lines) {
    ...
    if (m) {
      cur = { severity: m[1].toLowerCase(), criterion: m[2].replace(/\*\*/g, '').replace(/^[:\s]+/, '').split(/\s+[—–-]\s+/)[0].trim(), text: raw.trimEnd(), anchor: null, line: null, evidence: null };
      out.push(cur);
...
export function anchoredRevisionBlocks(verdictText, docText, { docs = {} } = {}) {
  ...
  for (const f of findings) {
    ...
    const locator = f.line ? { line: f.line } : fa ? fa.anchor : f.anchor;
    if (!locator) {
    ...
    const [slice] = sliceSections(source, [locator]);
    if (!slice.found) {
    ...
    const where = fa ? `${fa.file} → ${slice.anchor}` : slice.anchor;
