...
// The off-TTY fallback. It must also settle when stdin ENDS, not only when a
// line arrives: `rl.question`'s callback never fires at EOF, so a script that
// pipes fewer answers than there are prompts used to leave this promise pending
// forever — node then drained its event loop and exited 0, silently skipping
// every remaining step. A truncated `gspec install` that reports success and
// leaves the hooks uninstalled is the worst version of that. Running out of
// input now means "take the default", which is what a non-interactive caller
// means by staying quiet.
function ask(question) {
  // Two ways stdin can be exhausted, and both used to hang forever:
  //   • it ENDS while we wait      — 'close' fires, but rl.question's callback never does;
  //   • it ended BEFORE this call  — 'end' already fired, so 'close' never comes either.
  // The second is the common one: every prompt after the first shares one stdin,
  // so a script piping fewer answers than there are prompts hits it immediately.
  if (process.stdin.readableEnded || process.stdin.destroyed || process.stdin.closed) {
  ...
  return new Promise((resolve) => {
    ...
    rl.on('close', () => {
      if (!settled) process.stdout.write('\n'); // don't leave the prompt mid-line
      finish('');
    });
