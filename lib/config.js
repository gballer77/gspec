...
// exact names override for the load-bearing jobs — the system design, where a
// mistake propagates into everything downstream and no checker catches it.
//
// IMPLEMENTATION IS NOT ONE OF THEM, on Claude. It is the highest-volume agent
// in a build (per scope, per wave, and again on every continuation) and the one
// job whose output is checked by something DETERMINISTIC: `implementation-
// validator` runs verify.sh, so a weak result fails the gate rather than
// shipping. That is also why the validator itself sits in the cheap `qa` tier
// like the others — the hard signal is the test run, not the model reading it.
// By the time the implementer runs, the design is settled and what is left is
// following it. Codex keeps `gpt-5-codex` here because that is its
// code-specialized model, not its strong tier.
//
// A validator's tier follows HOW MUCH JUDGMENT IS LEFT AFTER ITS LINT. Most now
// sit behind a deterministic floor that has already settled the mechanical
// questions, so what remains is a focused read of one small file — cheap tier.
// `architecture-validator` is the exception: it has no lint behind it, and the
// two things it alone polices are pure judgment — that no feature-level detail
// crept in, and that the spec grew no sections of its own. A dogfood run showed
// exactly that gap, passing an architecture 1.7x its budget with five invented
// sections. It is the checker of the one spec that must stay finite, so it is
// pinned to the writer's own tier.
//
...
export const RECOMMENDED_MODELS = {
  claude: {
    ...
    'architecture-validator': 'claude-opus-5',
    'feature-architect': 'claude-opus-5',
    // implementer is NOT pinned up on Claude: it resolves to `default` (sonnet).
    // Listed here as a no-op only to say that plainly — it is the agent a reader
    // most expects to find on the strong tier, and its absence should read as a
    // decision rather than an omission. See the note above.
    implementer: 'claude-sonnet-5',
    // The one validator whose judgment provably failed on the cheap tier: it
    // read four rendered captures beside the design and passed a page in
    // browser-default layout. It runs once per build (~2M tokens), so the
    // balanced tier costs almost nothing here and it is the last gate.
    'implementation-validator': 'claude-sonnet-5',
  },
  codex: {
    default: 'gpt-5',
    writer: 'gpt-5',
    ...
    'architecture-validator': 'gpt-5-codex',
    'feature-architect': 'gpt-5-codex',
    implementer: 'gpt-5-codex',
    'implementation-validator': 'gpt-5',
  },
};
