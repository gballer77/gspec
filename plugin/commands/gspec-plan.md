...
1. **Resolve the feature** from the arguments below (its PRD at `gspec/features/<slug>.md`; if ambiguous, ask). If a plan file (`gspec/tasks/<slug>.md`) already exists and is non-empty, ask whether to (a) regenerate, (b) append tasks for newly added capabilities only, or (c) abort — do not overwrite without confirmation.
...
4. **Write** the approved `gspec/tasks/<slug>.md` (creating `gspec/tasks/` if needed; preserve any prior `spec-version`; new files use the current version). Never overwrite a non-empty file without the Step 1 confirmation.
