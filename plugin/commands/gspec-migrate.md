...
6. **Verify & report** — confirm every migrated file now carries the current `spec-version` and that no content was lost; summarize files migrated, files skipped (already current), any content relocated, and any `deployable` → `module` renames applied.
