...
1. **Read existing PRDs** in `gspec/features/` (to avoid overlap and find dependencies). Note the feature description from the arguments below.
...
4. **Write.** For each feature, delegate to the `feature-writer` agent with that feature's resolved brief → one PRD + summary. For a set, keep terminology and priorities consistent across PRDs and cross-link dependencies.
