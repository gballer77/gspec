...
4. **Write.** Delegate to the `architecture-writer` agent with the resolved gap decisions. It reads the specs and writes `gspec/architecture.md` (with Mermaid diagrams and the Technical Gap Analysis) — plus, for a multi-module system, one `gspec/architecture/<name>.md` per module (the architect skill's layout gate) — returning a summary.
