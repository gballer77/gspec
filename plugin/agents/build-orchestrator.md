...
- The project's gspec documents (read them yourself): `gspec/features/*.md` + `gspec/tasks/*.md` (capability + task checkboxes, `deps:`, `[P]`), and `gspec/architecture.md` (Project Structure, Modules — for the scaffold scope and file-overlap judgment; when `gspec/architecture/*.md` sub-files exist, their `module:`/`covers:` frontmatter and per-unit structure sharpen the file-overlap call — two scopes confined to different modules are file-disjoint).
