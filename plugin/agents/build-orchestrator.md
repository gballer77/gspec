...
- The project's gspec documents (read them yourself): `gspec/features/*.md` + `gspec/tasks/*.md` (capability + task checkboxes, `deps:`, `[P]`), and `gspec/architecture.md` (module boundaries + the Modules table — for the scaffold scope and file-overlap judgment; when `gspec/architecture/*.md` sub-files exist, each one's owned directories and placement rules sharpen the file-overlap call — two scopes confined to different modules are file-disjoint).
