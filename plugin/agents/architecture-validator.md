...
**Police the layout gate and the tier boundary** (the architect skill's Layout section): the layout matches the Modules table (one row → no sub-files; more than one → exactly one `architecture/<name>.md` per row, linked from its row, with `module:` frontmatter matching the row name and a `covers:` list); no cross-module concern (shared entity, inter-unit contract, cross-cutting auth, the Modules table itself) buried in a sub-file; no concern duplicated across tiers — duplication is drift and is a major finding.
