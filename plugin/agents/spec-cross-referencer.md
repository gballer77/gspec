...
Read (all-specs mode): `profile`, `stack`, `style` (`style.md` or `style.html`), `practices`, `architecture` (+ `architecture/*.md` sub-files when present), `research`, every `features/*.md`, and each `tasks/*.md`; note which features carry a `design.html`. (Scoped mode: the target PRD + its plan + the foundations only.)
